...
class BookingController extends Controller
{
    ...
    public function updateStatus(Request $request, Booking $booking): JsonResponse { abort_unless($booking->technician?->user_id===$request->user()->id,403); $data=$request->validate(['status'=>['required','in:accepted,rejected,completed']]); $booking->update(['status'=>$data['status']]); return response()->json(['booking'=>$booking]); }
